	...
	function endSong():Void
	{
		...
		else
		{
			if (isStoryMode)
			{
				...
				{
					if (curSong == "run" && storyDifficulty == 1)
					{
						FlxG.switchState(new EndingState());
					}
					else if (curSong == "run" && storyDifficulty == 2)
					{
						FlxG.switchState(new EndingState());
					}
					else
	...
	override function beatHit()
	{
		...
		if (curSong.toLowerCase() == 'onslaught' && curBeat >= 128 && curBeat <= 352 )
		{
			if (FlxG.random.bool(curBeat/10) && appearscreen)
			{
				...
				randomthing.loadGraphic(Paths.image('bob/PopUps/popup' + FlxG.random.int(1,3), 'shared'));
				...
				{
					FlxTween.tween(randomthing, {width: 1, alpha: 0}, 0.2, {ease: FlxEase.sineOut});
				}
				new FlxTimer().start(0.5 , function(tmr:FlxTimer)
				{
					appearscreen = true;
